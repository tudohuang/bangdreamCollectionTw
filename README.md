...
活動表的 `曲目` 記的是「這場唱了什麼」。要讓歌曲頁不只是統計，
再開一張分頁叫 **歌曲**，一列一首：

| 欄 | 說明 |
|---|---|
| `歌名` | 唯一必要的欄。跟曲目靠 `songKey` 對位，所以〜副標〜、（TV size）、全形半形不用手動統一 |
| `樂團` | 原唱。歌曲頁的標題底下會多一行，可以直接點進樂團頁 |
| `專輯` | 收錄專輯 |
| `發行` | `2023-9-27` 會自動補成 `2023-09-27`；看不懂的（「2023年秋」）原樣留著 |
| `作詞` `作曲` `編曲` | 一行一項顯示，沒填的不出現 |
| `連結` | 多條用空白或逗號分隔。Spotify、Apple Music、YouTube、KKBOX、Bandori.party、VGMdb 都認得出名字。**歌詞連結也貼這一欄** —— 歌ネット、UtaTen、J-Lyric 那類會自動分到「歌詞」那一排，不會混在「去哪裡聽」裡 |
| `封面` | 專輯封面網址。必須是 `http` 開頭，否則忽略；載不出來退回樂團色的唱片 |
| `別名` | 用 `、` 分隔。填了之後這幾種寫法都查得到同一首 |
| `備註` | 自由文字 |

**沒有「歌詞」這一欄，而且不會有。** 版權在 Bushiroad 與 JASRAC 底下 ——
歌詞站是付了授權金才能放全文的，粉絲站轉貼是最容易被直接寄信的一種內容。
而且歌詞到處查得到，「在台灣唱過幾次」才是只有這裡查得到的東西。
歌詞網址貼進 `連結` 欄就好，畫面上會自成一排「歌詞」。

空白表可以自動產：`npm run template 歌曲` 會抓線上 Sheet 的曲目，
輸出 `docs/template/歌曲.tsv` —— 已經照唱過次數排好，
「樂團」那欄能從歌單的 ▍區塊推出來的都先填好了。整張貼進新分頁就行。

> 這張分頁是**選填的**。沒建、建了空的、或抓不到，全站行為跟以前一模一樣 ——
> 歌曲頁少掉上半部，台灣履歷照常。整列只填歌名也不會長出一塊空白區域。

人物頁另外多了一塊**「這些場次唱過的歌」**，完全從既有的曲目推出來，不用填任何東西。
標題刻意不寫「他唱過的歌」：聯合場的歌單裡有別團的曲子，分不出來的部分就不假裝分得出來。
