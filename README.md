...
**最省事的做法：整份寫成一行，用分號 `;` 分隔。**

```
▍Ave Mujica; Choir 'S' Choir; 顔; ▍MyGO!!!!!; 回層浮; 無路矢; 安可; Returns
```

這樣一次 Ctrl+V 就好。

多行也可以（格子裡按 `Alt+Enter`），但**貼多行進 Google Sheet 很容易出事**：
點一下就貼會被拆成好幾**列**，要先雙擊進編輯模式才會進同一格；
中間漏一個換行，兩三首歌就會黏成一首，而畫面上只會看起來像
「這首歌的名字很長」，不會報錯。分號沒有這個問題。
...
官方公布的曲目長什麼樣就照貼 —— 「▍團名」分段、`M01.` 編號都認得。

> **歌名跟團名一樣的時候**（Ave Mujica 有一首歌就叫〈Ave Mujica〉）：
> 加編號就一定會被當成歌（`M01. Ave Mujica`）。
> 沒編號也救得回來 —— 已經在那一段裡了，同名再出現一次就是歌不是標頭。

最少只要一行一首歌名就夠了。另外三件事想標再標：

| 標法 | 意思 |
|---|---|
| `▍團名` 整行 | 區塊標頭，後面的歌都算那一團的。**雙團場用這個** —— 官方曲目本來就這樣分段 |
| `歌名／團名` | 單獨指定某一首誰唱的。單團場不用標（自動帶上）；兩團以上沒標就留空，不猜 |
| 單獨一行 `安可` | 後面的都算安可。第二次寫 `W安可` 或 `EN2` |
| `MC`、`影片`、`樂器演奏` | 不是歌，不會進「這首唱過幾次」的統計，但會留在曲目裡 |
| `【Day 1】` | 一列涵蓋兩天的場次（8/01–8/02 那種），兩份歌單塞同一格時分天。編號每天重來 |
| `みか 部分`、`合唱` | 誰唱的那一段。暱稱原樣留著 —— 「みか」跟名冊上的「小日向美香」對不起來，猜不如照實記 |
| 歌名後的 `（…）` | 出處註記（「動畫《K-ON!》插入曲」）。拆出來單獨顯示，不混進歌名 —— 不然同一首換個註記就變兩首 |

團名有空白也認得（`Ave Mujica`、`RAISE A SUILEN`）。
認不出是樂團的話就不拆 —— 寧可不標，也不要把歌名切斷。

**這幾件事不用你打，填了曲目就自動有：**
台灣首唱、開場幾次、收尾幾次、安可幾次、這首唱過幾場、兩場的重疊度、
每場平均幾首、最常拿來開場的是哪一首。

> 曲目、周邊、場刊這種一整串的，在 Sheet 的格子裡按 `Alt+Enter` 換行就好 ——
> 多行格子匯出成 CSV 時會被雙引號包起來，解析器吃得到（有測試守著）。
> 想在編輯器裡慢慢寫的話走 `content/impressions/<ID>.md`，但那條路要重新部署才會生效。
