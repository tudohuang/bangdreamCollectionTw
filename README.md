...
| `連結` | 多條用空白或逗號分隔。Spotify、Apple Music、YouTube、KKBOX、歌詞站、Bandori.party、VGMdb 都認得出名字 |
| `封面` | 專輯封面網址。必須是 `http` 開頭，否則忽略；載不出來退回樂團色的唱片 |
| `別名` | 用 `、` 分隔。填了之後這幾種寫法都查得到同一首 |
| `備註` | 自由文字 |

**歌詞只放連結，不放本文。** 版權在 Bushiroad 與 JASRAC 底下，轉貼全文是最容易
出事的一種內容；而且歌詞到處查得到，「在台灣唱過幾次」才是只有這裡查得到的東西。
連到 uta-net、UtaTen 那類站，連結會自動標成「歌詞」。
