...
// 詳情浮層 = 一張「後台通行證」：舞台頭圖、巨型編號水印、撕票線、ADMIT ONE 微型字＋條碼
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        className="relative w-full sm:max-w-3xl max-h-[94vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl border border-dream-line shadow-glassHover scrollbar-thin dark:border-white/15 focus:outline-none"
      ...
      >
        ...
        {/* ---------- 撕票線：ADMIT ONE 微型字 + 條碼 ---------- */}
        <div className="flex items-center justify-between gap-3 px-5 sm:px-8 py-2.5 border-b border-dashed border-dream-line dark:border-white/15 text-dream-faint">
        ...
        <div className="px-5 sm:px-8 pb-0 pt-5 sm:pt-6 relative">
          {personal && (
          ...
          <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 gap-2.5">
            ...
            {event.venue && (
              <FactTile icon="location-dot" label="地點" color={meta.color} glow={meta.glow}
                ...
                <span className="text-[12px] text-bloom-violet mt-0.5 inline-flex items-center gap-1">在地圖開啟<Icon n="link" className="text-[9px]" /></span>
              </FactTile>
            )}
          ...
          {/* 聲優 / 樂團 / 主辦 / 購票 / 備註：收進一張資訊卡，虛線分隔延續票根語言 */}
          <div className="mt-3 rounded-2xl border border-dream-line dark:border-white/10 overflow-hidden divide-y divide-dashed divide-dream-line dark:divide-white/10">
              ...
              {people.length ? <span className="flex flex-wrap gap-1.5">
                {people.map(p => (
                  <a key={p} href={`#/person/${encodeURIComponent(p)}`}
                    className="rounded-full bg-white/70 dark:bg-white/10 border border-dream-line px-2.5 py-0.5 text-[13px] hover:border-bloom-violet transition-colors">{p}</a>
                ))}
            ...
            {event.ticketUrl && <InfoLine icon="link" term="購票" color={meta.color} glow={meta.glow}>
              <a className="text-bloom-violet hover:underline break-all text-[13.5px]" target="_blank" rel="noopener noreferrer" href={event.ticketUrl}>{event.ticketUrl}</a>
            </InfoLine>}
                ...
                {galleryPhotos.map((p, i) => {
                  ...
                  return (
                    <button key={i} onClick={() => setLightbox(url)}
                      className="aspect-[4/3] overflow-hidden rounded-lg border border-dream-line" aria-label={`放大照片 ${i + 1}`}>
          ...
          <div className="sticky bottom-0 mt-8 -mx-5 sm:-mx-8 px-5 sm:px-8 py-3 flex flex-wrap gap-2.5 items-center border-t border-dream-line dark:border-white/10"
            style={{ background: 'var(--modal-bg)' }}>
            <button className="btn-primary !h-10" onClick={copyLink}>
              <Icon n="link" /> 複製連結
            </button>
            <button className="pill !px-4 !py-2 !text-[13.5px]" onClick={copySummary}><Icon n="clipboard" /> 摘要</button>
            <button className="pill !px-4 !py-2 !text-[13.5px]" onClick={() => downloadShareImage(event, meta, personal)}><Icon n="star" /> 存成圖</button>
  ...
  const inner = (
    <>
      <div className="flex items-center gap-1.5 mb-1.5">
        <span className="grid place-items-center w-5 h-5 rounded-md text-[10px] text-white" style={{ background: color }}><Icon n={icon} /></span>
