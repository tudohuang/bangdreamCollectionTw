...
export default function VenueMap({ events }) {
  ...
  return (
    <div>
      ...
      <p className="mt-2.5 text-[14px] text-dream-sub">
      ...
      {located.length === 0 ? (
        <div className="glass mt-5 px-6 py-14 text-center">
          ...
          <p className="mt-2 text-[13.5px] text-dream-sub max-w-md mx-auto leading-relaxed">
            ...
            {located.map(v => {
              ...
              return (
                <a key={v.venue} href={`#/collection?venues=${encodeURIComponent(v.venue)}`}
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[12.5px] transition-colors"
                  style={on
      ...
      {far.length > 0 && (
        <div className="mt-4 rounded-2xl border border-dream-line dark:border-white/10 px-5 py-4">
          <div className="text-[12.5px] font-bold text-dream-faint mb-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {far.map(v => {
              ...
              return (
                <a key={v.venue} href={`#/collection?venues=${encodeURIComponent(v.venue)}`}
                  className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[12.5px] text-dream-sub border border-dream-line dark:border-white/10 hover:border-bloom-violet transition-colors"
      ...
      {missing.length > 0 && (
        <div className="mt-4 rounded-2xl border border-dashed border-dream-line dark:border-white/15 px-5 py-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {missing.map(v => (
              <span key={v.venue} className="rounded-full border border-dream-line dark:border-white/10 px-2.5 py-1 text-[12.5px] text-dream-sub">
                {v.venue}<span className="text-dream-faint"> ×{v.count}</span>
              </span>
            ))}
          </div>
