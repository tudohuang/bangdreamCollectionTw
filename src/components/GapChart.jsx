...
export default function GapChart({ events, onSelect, today = new Date() }) {
  ...
  return (
    <div className="glass p-6">
      <h3 className="font-display font-bold text-[16px] text-dream-ink">最長的等待</h3>
        ...
        {list.map(g => {
          ...
          return (
            <li key={g.prev.id + g.next.id}>
              ...
              <div className="mt-1 flex items-center gap-2 text-[14px] text-dream-sub">
      ...
      {now && (
        <p className="mt-4 pt-4 border-t border-dream-line dark:border-white/10 text-[14px] text-dream-sub">
