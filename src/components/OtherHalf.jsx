...
export default function OtherHalf({ events }) {
  ...
  return (
    <div>
      ...
      <p className="mt-2.5 text-[16px] text-dream-sub">排行榜只講來最多的那幾個。這裡是剩下的那一半。</p>
        ...
        {once.length > 0 && (
          <div className="glass p-6 sm:p-7">
            <h3 className="flex items-center gap-2.5 font-display font-bold text-lg text-dream-ink">
              ...
              <span className="ml-auto font-round text-[16px] text-dream-faint">{once.length} 位</span>
            </h3>
            <p className="mt-1.5 font-hand text-[16px] text-dream-faint">來過台灣，然後就沒有然後了。</p>
            <div className="mt-4 flex flex-wrap gap-1.5">
              {once.map(p => (
                <a key={p} href={`#/person/${encodeURIComponent(p)}`}
                  className="rounded-full border border-dream-line dark:border-white/10 bg-white/70 dark:bg-white/[.06] px-3 py-1 text-[14px] text-dream-ink hover:border-bloom-violet transition-colors">
                  {p}
                </a>
              ))}
            </div>
        ...
        {absent.length > 0 && (
          <div className="glass p-6 sm:p-7">
            <h3 className="flex items-center gap-2.5 font-display font-bold text-lg text-dream-ink">
              ...
              <span className="ml-auto font-round text-[16px] text-dream-faint">{absent.length} 團</span>
            </h3>
            <p className="mt-1.5 font-hand text-[16px] text-dream-faint">只有聲優以個人身分來過，全團還沒。</p>
            <div className="mt-4 flex flex-wrap gap-2">
              {absent.map(b => {
                const m = bandMeta(b)
                return (
                  <a key={b} href={`#/band/${encodeURIComponent(b)}`}
                    className="inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-[14px] font-medium hover:opacity-80 transition-opacity"
